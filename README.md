...
기능적 요구사항
1. 고객이 동영상을 업로드한다
1. 고객이 업로드한 동영상을 수정/삭제 할 수 있다

1. 고객이 채널을 생성한다
1. 고객이 채널을 수정/삭제 할수있다

1. 고객이 환급신청한다
1. 정책관리시스템이 환급정책을 확인하여 환급한다

1. 관리자가 정책을 등록한다
1. 관리자가 정책을 수정할 수 있다
1. 정책확인요청을 받으면 정책확인 후 위반 시 동영상 삭제 요청한다


비기능적 요구사항
1. 트랜잭션
    1. 동영상이 등록/수정 시 정책에 맞는지 확인되어야 한다        Async 호출 
    1. 동영상이 등록/수정/삭제 시 채널 정보 업데이트가 되어야한다  Async 호출
    1. 고객이 환급신청시 환급 정책에 맞는지 확인되어야 한다        Async 호출
    1. 채널이 삭제되면 채널에 속한 동영상이 삭제되어야 한다        Async 호출
1. 장애격리
    1. 고객관리 기능이 수행되지 않더라도 동영상 서비스는 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    1. 동영상관리서비스 사용이 과중되면 사용자를 잠시동안 받지 않고 잠시후에 하도록 유도한다  Circuit breaker, fallback
1. 성능
    (수정필요)1. 고객이 자주 상점관리에서 확인할 수 있는 배달상태를 주문시스템(프론트엔드)에서 확인할 수 있어야 한다  CQRS
    (수정필요)1. 배달상태가 바뀔때마다 카톡 등으로 알림을 줄 수 있어야 한다  Event driven
