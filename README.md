...
1. 고객이 채널을 생성한다
2. 고객이 채널을 수정/삭제 할수있다

3. 고객이 동영상을 업로드한다
4. 고객이 업로드한 동영상을 수정/삭제 할 수 있다
5. 고객이 환급신청한다
6. 정책관리시스템이 환급정책을 확인하여 환급한다

7. 관리자가 정책을 등록한다
8. 관리자가 정책을 수정할 수 있다
9. 정책확인요청을 받으면 정책확인 후 위반 시 동영상 삭제 요청한다
...
1. 트랜잭션
    ...
    4. 채널이 삭제되면 채널에 속한 동영상이 삭제되어야 한다         Async 호출
1. 장애격리
    1. 고객관리 기능이 수행되지 않더라도 동영상 서비스는 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    2. 동영상 관리 서비스 사용이 과중되면 사용자를 잠시동안 받지 않고 잠시 후에 하도록 유도한다  Circuit breaker, fallback
1. 성능
    1. 고객이 자주 채널관리시스템에서 동영상 Total 조회수를 확인할 수 있어야 한다  CQRS
    2. 정책에 따라 동영상의 상태가 변경될 때마다 카톡 등으로 알림을 줄 수 있어야 한다  Event driven
...
![image](https://user-images.githubusercontent.com/63624229/81649462-4ba90100-946b-11ea-9b55-243ef71c725c.png)
